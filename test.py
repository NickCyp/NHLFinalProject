                ...
                for play in pbp['plays']:
                    if play.get('typeDescKey') == 'hit':
                        ...
                        if hitter_id and hittee_id:
                            try:
                                cursor.execute('''
                                INSERT INTO hits (game_id, period, period_type, time_elapsed, hitter_id, hittee_id)
                                VALUES (?, ?, ?, ?, ?, ?)
                                ''', (
                                    game_data.get('id'),
                                    period_number,
                            ...
                            print(f"Skipping hit record - missing player IDs: hitter_id={hitter_id}, hittee_id={hittee_id}")
    except RequestException as e:
